...
> GROUP BY is a SQL clause used to group rows that have the same values in one or more columns, allowing aggregate functions like COUNT(), SUM(), AVG(), MIN(), and MAX() to perform calculations on each group instead of the entire table.
